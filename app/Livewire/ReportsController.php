...
class ReportsController extends Component
{
    ...
    public function render()
    {
        ...
        $this->SalesByDate();//ventas por fecha
    ...
    public function SalesByDate() // este metodo retornara un reporte de las ventas por fecha
    {
        if($this->reportType == 0) // ventas del dia
        {
            ...
            $to = Carbon::parse(Carbon::now())->format('Y-m-d')   . ' 23:59:59';//hasta esa hora del mismo día
             ...
             $from = Carbon::parse($this->dateFrom)->format('Y-m-d') . ' 00:00:00';//el usuario selecciona desde esta fecha 
             $to = Carbon::parse($this->dateTo)->format('Y-m-d')     . ' 23:59:59';//hasta esta fecha
        }
        ...
        if($this->userId == 0) // entonces es de todos los usuarios
        {
